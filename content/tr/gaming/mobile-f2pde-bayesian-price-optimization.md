...
**Multi-armed bandit integration:** IAP ladder birden fazla SKU içeriyorsa (starter pack $4.99, mega pack $19.99, ultimate $49.99), Thompson Sampling production'da bandit algoritması olur. Her impression'da her SKU için posterior sample çekilir, maksimum expected revenue veren sunulur. Bu [App Store Optimization](https://www.roibase.com.tr/tr/aso) çalışmalarıyla birleştirildiğinde güçlü bir monetization engine oluşur — ASO traffic'i doğru segment'e yönlendirir, Bayesian pricing o segment'e optimal IAP'yi sunar.

**Monitoring ve alert:** Posterior distribution'da ani değişim varsa (örneğin credible interval 3 gün içinde %50 genişledi) bu anomaly sinyali — platform bug, competitor kampanya, seasonal effect olabilir. Alert sistemini posterior variance üzerine kurun:

```python
# Posterior variance izleme
...
Mobile F2P'de price optimization artık "bir kez test et, sonsuza kadar kullan" değil — Bayesian posterior estimation ile sürekli refine edilen, segment-spesifik, dinamik bir sistem. IAP ladder'ınızı frequentist mantıkla test ediyorsanız, whale segment revenue'sunu masada bırakıyorsunuz. Bayesian framework hem test süresini kısaltır hem segment-bazlı optimizasyon sağlar hem de production'da sürekli öğrenen bir pricing engine kurar. Oyununuzun conversion rate'i %3'te takılıysa, sorun fiyat değil — fiyatı kim, ne zaman, hangi context'te görüyor? Posterior distribution bu sorunun yanıtını verir.